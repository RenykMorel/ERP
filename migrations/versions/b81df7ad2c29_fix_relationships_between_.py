"""Fix relationships between InventarioItem and ItemPreFactura

Revision ID: b81df7ad2c29
Revises: 6581e43138ff
Create Date: 2024-10-06 14:31:54.145370
...
revision = 'b81df7ad2c29'
down_revision = '6581e43138ff'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Función auxiliar para verificar si una restricción existe
    def constraint_exists(table, constraint):
        return any(cons['name'] == constraint for cons in inspector.get_foreign_keys(table))

    # Función auxiliar para verificar si una columna existe
    def column_exists(table, column):
        return column in [col['name'] for col in inspector.get_columns(table)]

    # Eliminar restricciones de clave foránea solo si existen
    if constraint_exists('ajustes_inventario', 'ajustes_inventario_item_id_fkey'):
        op.drop_constraint('ajustes_inventario_item_id_fkey', 'ajustes_inventario', type_='foreignkey')
    
    if constraint_exists('items_factura', 'items_factura_item_id_fkey'):
        op.drop_constraint('items_factura_item_id_fkey', 'items_factura', type_='foreignkey')
    
    if constraint_exists('items_pre_factura', 'items_pre_factura_item_id_fkey'):
        op.drop_constraint('items_pre_factura_item_id_fkey', 'items_pre_factura', type_='foreignkey')
    
    if constraint_exists('movimientos_inventario', 'movimientos_inventario_nota_credito_id_fkey'):
        op.drop_constraint('movimientos_inventario_nota_credito_id_fkey', 'movimientos_inventario', type_='foreignkey')
    
    if constraint_exists('movimientos_inventario', 'movimientos_inventario_factura_id_fkey'):
        op.drop_constraint('movimientos_inventario_factura_id_fkey', 'movimientos_inventario', type_='foreignkey')
    
    if constraint_exists('movimientos_inventario', 'movimientos_inventario_item_id_fkey'):
        op.drop_constraint('movimientos_inventario_item_id_fkey', 'movimientos_inventario', type_='foreignkey')

    # Crear la tabla items_inventario solo si no existe
    if 'items_inventario' not in inspector.get_table_names():
    ...
    # Crear nuevas relaciones de clave foránea
    if not constraint_exists('ajustes_inventario', 'ajustes_inventario_item_id_fkey'):
        op.create_foreign_key(None, 'ajustes_inventario', 'items_inventario', ['item_id'], ['id'])
    
    if not constraint_exists('items_factura', 'items_factura_item_id_fkey'):
        op.create_foreign_key(None, 'items_factura', 'items_inventario', ['item_id'], ['id'])
    
    if not constraint_exists('items_pre_factura', 'items_pre_factura_item_id_fkey'):
        op.create_foreign_key(None, 'items_pre_factura', 'items_inventario', ['item_id'], ['id'])
    
    if not constraint_exists('movimientos_inventario', 'movimientos_inventario_item_id_fkey'):
        op.create_foreign_key(None, 'movimientos_inventario', 'items_inventario', ['item_id'], ['id'])

    # Eliminar columnas solo si existen
    with op.batch_alter_table('movimientos_inventario', schema=None) as batch_op:
        if column_exists('movimientos_inventario', 'factura_id'):
            batch_op.drop_column('factura_id')
        if column_exists('movimientos_inventario', 'nota_credito_id'):
